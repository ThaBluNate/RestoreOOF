...
def select():
    global filename
    filename = filedialog.askopenfilename(title='Select Sound',initialdir=pict,filetypes=(('OGG sound files','*.ogg'),))
    if filename == '':
...
Style=ttk.Style()
Style.theme_use('alt')
Style.configure("TLabel",font=('corbel',12),background="#333",foreground='#ddd')
Style.configure("TButton",font=('corbel',12),background="#333",foreground='#ddd')
Style.configure("TFrame",background="#333")
Style.configure("TNotebook",background="#111")
Style.configure("TNotebook.Tab",background='#222',foreground='#ddd');
Style.map("TNotebook.Tab",background=[("selected",'#333')],foreground=[("selected",'#ddd')]);
Style.map('TButton',background=[('active','#111'),('disabled','#111')],foreground=[('disabled','#555')])
...
Label(T,image=logoimage,background='#111',foreground='#ddd').grid(column = 0,row = 0,sticky='W')
Label(T,text='Restore oof sound',background='#111',foreground='#ddd',font=('corbel',12)).grid(column=0,row=0)
Label(T,image=logoimage,background='#111',foreground='#ddd').grid(column = 0,row = 0,sticky='E')
